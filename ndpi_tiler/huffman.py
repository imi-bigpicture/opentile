...
class HuffmanNode:
    ...
    def __init__(self, length: int) -> None:
        ...
        self._insertion_order: List[
            Callable([HuffmanLeaf, int], Optional[int])
        ] = [
    ...
    def insert(
        ...
        """
        for insertion_function in self._insertion_order:
...
class HuffmanTable:
    ...
    def _to_bits(value: int, length: int) -> bitarray:
        ...
        if length == 0:
            return bitarray()
        return int2ba(value, length=length)

    def encode_value(self, value: int) -> bitarray:
        length, code = self._code_value(value)
        output = self.encode_into_bits(length)
        output += self._to_bits(code, length)
        return output
