...
"""Image implementation for Mikroscan tiff files."""
...
class MikroscanTiffImage(NativeTiledTiffImage, LevelTiffImage):
    def __init__(
        self, page: TiffPage, file: OpenTileFile, base_size: Size, base_mpp: SizeMm
    ):
        """Level image for a Mikroscan tiff file. The level is natively tiled (jpeg),
        so the tiles are served as-is.
...
class MikroscanSingleFrameImage(BaseTiffImage):
    """A Mikroscan image stored as an uncompressed, multi-strip frame (thumbnail,
    label, macro). The strips are read and assembled into one raw-pixel tile; there is
    no per-tile encoded representation to pass through, so it is served uncompressed."""
...
class MikroscanAssociatedImage(MikroscanSingleFrameImage, AssociatedTiffImage):
    """Associated image (label or macro) of a Mikroscan tiff file."""
...
class MikroscanThumbnailImage(MikroscanSingleFrameImage, ThumbnailTiffImage):
    def __init__(
        self, page: TiffPage, file: OpenTileFile, base_size: Size, base_mpp: SizeMm
    ):
        """Thumbnail image of a Mikroscan tiff file.
