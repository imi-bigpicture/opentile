...
"""Shared image implementations.

Reading strategies (`BaseTiffImage` and friends) and the ready-made combinations of
a strategy with a role from `opentile.tiff_image` that formats instantiate.
"""
...
class NativeTiledLevelImage(NativeTiledTiffImage, LevelTiffImage):
    ...
    pyramid index, and pixel spacing derived from the base level, for formats that need
    only this plain passthrough level."""
...
class NativeTiledAssociatedImage(NativeTiledTiffImage, AssociatedTiffImage):
    """Natively tiled associated image (label or macro) with no defined pixel spacing,
    served as-is."""
...
class NativeTiledThumbnailImage(NativeTiledTiffImage, ThumbnailTiffImage):
    """Natively tiled thumbnail served as-is, scaled from the pyramid base level."""
...
class NativeSingleFrameImage(BaseTiffImage, metaclass=ABCMeta):
    """Meta class for a page stored as a single frame, served as one tile.

    The stored frame is passed through untouched, whatever its compression (e.g. an lzw
    label), the single-frame counterpart of `NativeTiledTiffImage`. Contrast
    `DecodedSingleFrameImage`, which decodes the page instead of passing it through.
    """
...
class SingleFrameAssociatedImage(NativeSingleFrameImage, AssociatedTiffImage):
    """Associated image stored as a single frame, served as one tile.

    Used by formats whose label is written as one frame rather than as strips.
    """
